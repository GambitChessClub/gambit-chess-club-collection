...
export class Collections {
  ...
  mint(args: collections.mint_arguments): collections.empty_object {
    ...
    for (let index = 0; index < <i32>args.number_tokens_to_mint; index++) {
      ...
      //hash used to randomly select the next token. the hash is based on the to address, the number of tokens to be minted and the amount of tokens already minted
      let hash = 0;
      ...
      //generate candidate token index (from 1 until the amount of tokens still available for minting)
      const indexCandidate = (hash % (Constants.MAX_SUPPLY - (index + supply.value)) + 1);

	  // variable used to count the tokens ids that are still available for minting
	  ...
	  //if the index for the new token is not found, the minting will fail
    ...
    // check limit address
    System.require(balance.value <= 151, "exceeds the limit of tokens per address");
  ...
  transfer(args: collections.transfer_arguments): collections.empty_object {
    ...
    let isTokenApproved: boolean = false;
